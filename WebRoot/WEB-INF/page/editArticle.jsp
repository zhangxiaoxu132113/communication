...
<html>
	<head>
	    ...
	    window.onload = function(){
	        ...
	        closeTag();//关闭标签的div
	        getTags();//获取标签的值
	        //从request中获取content值，并回显
	       // alert("haha");
	        var contents = document.getElementsByName("content");
	        var content1 = document.getElementById("hiddenContent").innerHTML;
	        contents[0].value= content1;
		...
		function sendBlog(){
			document.getElementById("sendBlog").onclick=function(){
				document.form2.action="blog/blogAction_editArticle.do";
				alert("add bLOG");
	...
	<body>
			...
			<form name="form2" id="form2" method="post">
					...
					<div id="article_top">
						<label class="article_top_keyword">标题</label><s:textfield name="title" id="title"></s:textfield>
						<label class="article_top_keyword">创建时间</label><s:textfield name="createTime"></s:textfield>
					...
					<div id="tag_div">
						...
						<div class="more_table_cell" id="tag_content">
							<s:textfield name="keywords" id="tags"></s:textfield><label id="addTags" onclick="showBg();" >add tags</label>
							<div id="showTag">
					...
					<div id="article_form">
						<input type="button" id="sendBlog" name="sendBlog" value="更新" />
						<input type="button" id="save" name="save" value="保存到草稿箱"/>
					</div>
					<s:hidden name="article_id"></s:hidden>
			</form>
			</div>
